            ...
            <div class="box">
                <div class="box-header">
                    <a href="javascript: ;" onclick="addModal()" class="btn btn-success">
                        <span class="hidden-xs"> @lang('core.btnAddCategory') </span><i class="fa fa-plus"></i>
                    </a>
                </div>
                <!-- /.box-header -->
                <div class="box-body">
...
//        Show Add modal
        function addModal() {
            $.ajaxModal('#addEditModal','{{route('admin.categories.create')}}');
        }
...
//        Update User Function ajax request
        function addEditCategory(id) {
            ...
            $.easyAjax({
                type: method,
                url: url,
                data: $('#add-edit-form1').serialize(),
                container: "#add-edit-form1",
                success: function(response) {
                    if (response.status == "success") {
